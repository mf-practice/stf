...
module.exports = function(options) {
  ...
  app.post('/s/upload/:plugin', function(req, res) {
    ...
    // Мерзкая библиотечка у которой не работает maxFiles (https://github.com/node-formidable/formidable/issues/901)
    let countFileUploaded = 0
    form.on('fileBegin', function(name, file) {
      if(countFileUploaded > 0){
        form.emit('error', new TooManyFiles())
        return
      }
    ...
    Promise.promisify(form.parse, form)(req)
      ...
      .catch(TooManyFiles, err => {
        res.status(405).json({success: false, description: 'Too many files'})
      })
      .catch(err => {
        // А ещё мерзкая библиотечка не умеет в типы ошибок
        if(err.cause.message.startsWith('maxFileSize exceeded')) {
          res.status(413).json({success: false, description: 'File is too big'})
        }
        else {
          throw err
        }
