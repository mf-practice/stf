...
function createFileSizeValidationMiddleware(options) {
  return (req, res, next) => {
    ...
    if (bytesExpected > options.maxSize) {
      res.status(413).json({
        success: false,
        ...
        return await new Promise(function (resolve, reject) {
          ...
          .then(device => {
            ...
            req.options.push.send([
              device.channel, wireutil.transaction(responseChannel, new wire.InstallMessage(downloadHref, launch, JSON.stringify(manifest)))
            ])
